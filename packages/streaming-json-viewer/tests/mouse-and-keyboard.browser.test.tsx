...
describe('mouse and keyboard', () => {
  ...
  test.fails(
    'focus-visible survives scrolling the focused row offscreen and back',
    async () => {
      ...
      // Same row should still own DOM focus and focus-visible. Today the
      // remount drops focus to body and `effectiveFocusedId` falls back to
      // the first rendered row, dropping the focus-visible highlight on
      // `title`. This assertion documents the bug; flip `test.fails` →
      // `test` when it's fixed.
